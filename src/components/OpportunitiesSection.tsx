...
export function OpportunitiesSection() {
  ...
  const handleAddOpportunity = () => {
    if (!newOpportunity.name.trim() || !newOpportunity.company.trim()) return;

    const opportunity: Opportunity = {
      id: Date.now().toString(),
      ...newOpportunity,
      requirements: newOpportunity.requirements.split(',').map(req => req.trim()),
      period: {
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value: any) => setNewOpportunity(prev => ({ ...prev, level: value }))}
                  >
                    <SelectTrigger className="bg-slate-700 border-slate-600 text-white mt-1">
                      <SelectValue />
                    </SelectTrigger>
